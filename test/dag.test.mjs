...
test("independent branches start concurrently; join waits for both; results only reach dependents; nodes run once", async () => {
  ...
  const result = executeDag(
    [
      ...
      },
      {
        ...
        dependsOn: ["root"],
        async run(_, deps) {
          assert.deepEqual(deps, { root: "source" });
          events.push("a");
          await a.promise;
          return "A";
        },
  ...
  await tick();
  assert.deepEqual(events, ["root", "a", "b"]);
  a.resolve();
  await tick();
  assert.deepEqual(events, ["root", "a", "b"]);
  b.resolve();
...
test("failed required branch blocks downstream join and final node", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.deepEqual(
        error.execution.nodes.map((n) => n.status),
        ["failed", "completed", "blocked", "blocked"],
      );
      return true;
    },
  );
...
test("duplicate IDs, missing dependencies and cycles are rejected", () => {
  ...
  assert.throws(
    () => validateGraph([node("a", []), node("a", [])]),
    /Duplicate/,
  );
  assert.throws(() => validateGraph([node("a", ["missing"])]), /Missing/);
  assert.throws(
    () => validateGraph([node("a", ["b"]), node("b", ["a"])]),
    /Cycle/,
  );
});
test("execution has a bounded deadline even if a node ignores cancellation", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      ...
      assert.equal(error.execution.nodes[1].status, "blocked");
      return true;
    },
  );
...
test("concurrent request input and output state are isolated", async () => {
  ...
  const results = await Promise.all([
    executeDag(nodes, { value: "first" }),
    executeDag(nodes, { value: "second" }),
  ]);
  assert.equal(results[0].outputs.one, "first");
