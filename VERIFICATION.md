...
- First public model request returned HTTP 502 after one required branch failed; downstream nodes were blocked, text stayed visible, and no task was added. Manual retry returned HTTP 200 with all five nodes completed in 3.41 seconds. The app makes no canned-success substitution. Later sanitized production diagnostics identified the intermittent failures as upstream HTTP 429. The final configuration allows one bounded SDK retry per branch for transient HTTP errors, within the existing 45-second workflow deadline. Persistent upstream failure remains possible.
