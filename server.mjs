...
export function createApp({ workflow = createWorkflow(generate), limit = 20 } = {}) {
  let active = 0;
  let requests = [];
  return http.createServer(async (req, res) => {
    ...
    if (req.method === 'GET' && path === '/health') return send(200, { status: 'ok', service: 'daywell', commit: process.env.APP_COMMIT || 'local' });
    if (req.method === 'GET' && staticFiles.has(path)) {
      ...
      } catch { send(500, { error: 'The page could not load. Please refresh.' }); }
      return;
    }
    if (req.method !== 'POST' || path !== '/api/understand') return send(404, { error: 'Page not found.' });
    if (!req.headers['content-type']?.startsWith('application/json')) return send(415, { error: 'Please send a JSON message.' });
    if (req.headers['sec-fetch-site'] === 'cross-site') return send(403, { error: 'Please use Daywell directly.' });
    ...
    if (requests.length >= limit || active >= 2) return send(429, { error: 'Daywell is busy. Please wait a minute and try again.', retryable: true }, { 'Retry-After': '60' });
    requests.push(now); active++;
    const controller = new AbortController();
    const disconnect = () => { if (!res.writableEnded) controller.abort(); };
    res.on('close', disconnect);
    const timer = setTimeout(() => { controller.abort(); send(408, { error: 'This took too long. Your message is still here. Please try again.', retryable: true }); }, 50000);
    try {
      let length = 0; const chunks = [];
      for await (const chunk of req) {
        length += chunk.length;
        if (length > 20000) { send(413, { error: 'Please shorten your message to 4,000 characters or fewer.' }); req.resume(); return; }
        chunks.push(chunk);
      }
      let body;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  server.listen(Number(process.env.PORT) || 8080, '0.0.0.0', () => console.log('Daywell listening'));
}
