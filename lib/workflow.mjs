...
export function createWorkflow(generate) {
  const nodes = [
    ...
    {
      id: "explain_and_extract",
      dependsOn: ["validate_input"],
      async run({ signal }, deps) {
        return validateExplanation(
          await generate({
            message: deps.validate_input.message,
            locale: deps.validate_input.locale,
    ...
    {
      id: "compose_plan",
      dependsOn: ["explain_and_extract", "review_safety"],
      async run({ input }, deps) {
        return composePlan(
          deps.explain_and_extract,
          deps.review_safety,
          input.locale,
        );
      },
    },
    {
      ...
      async run(_, deps) {
        return validatePlan(deps.compose_plan);
      },
    },
  ];
  return async (message, options = {}) => {
    const { outputs, execution } = await executeDag(
      nodes,
      { message, locale: options.locale || "hi" },
      options,
    );
    return {
      plan: outputs.validate_output,
