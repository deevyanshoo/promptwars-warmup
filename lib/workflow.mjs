...
export function createWorkflow(generate) {
  const nodes = [
    { id: 'validate_input', dependsOn: [], async run({ input }) { return validateInput(input.message); } },
    { id: 'explain_and_extract', dependsOn: ['validate_input'], async run({ signal }, deps) {
      return validateExplanation(await generate({ message: deps.validate_input, prompt: explanationPrompt, schema: explanationSchema, signal }));
    } },
    { id: 'review_safety', dependsOn: ['validate_input'], async run({ signal }, deps) {
      return validateSafety(await generate({ message: deps.validate_input, prompt: safetyPrompt, schema: safetySchema, signal }));
    } },
    { id: 'compose_plan', dependsOn: ['explain_and_extract', 'review_safety'], async run(_, deps) {
      return composePlan(deps.explain_and_extract, deps.review_safety);
    } },
    { id: 'validate_output', dependsOn: ['compose_plan'], async run(_, deps) { return validatePlan(deps.compose_plan); } },
  ];
  return async (message, options) => {
    const { outputs, execution } = await executeDag(nodes, { message }, options);
    return { plan: outputs.validate_output, execution };
  };
}
