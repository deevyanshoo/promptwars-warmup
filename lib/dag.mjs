...
export async function executeDag(
  ...
  const state = new Map(
    nodes.map((n) => [n.id, { id: n.id, status: "pending", durationMs: 0 }]),
  );
  ...
  const metadata = () => ({
    ...
    nodes: [...state.values()].map((s) => ({ ...s })),
  });
  try {
    while (
      [...state.values()].some((s) => ["pending", "running"].includes(s.status))
    ) {
      for (const node of nodes) {
        ...
        if (
          node.dependsOn.some((id) =>
            ["failed", "blocked"].includes(state.get(id).status),
          )
        ) {
          record.status = "blocked";
          continue;
        }
        if (!node.dependsOn.every((id) => state.get(id).status === "completed"))
          continue;
        ...
        const dependencies = Object.freeze(
          Object.fromEntries(
            node.dependsOn.map((id) => [id, structuredClone(results.get(id))]),
          ),
        );
        const promise = Promise.resolve()
          .then(() => node.run(context, dependencies))
          .then(
            (output) => {
              if (record.status !== "running") return;
              results.set(node.id, structuredClone(output));
          ...
          .finally(() => {
            if (record.status !== "failed" || record.durationMs === 0)
      ...
      if (
        ...
        (running.size &&
          (await Promise.race([...running.values(), deadline])) === "timeout")
      ) {
      ...
      if (
        !running.size &&
        ![...state.values()].some((s) => s.status === "pending")
      )
        break;
    }
    ...
    const sinks = nodes.filter(
      (n) => !nodes.some((other) => other.dependsOn.includes(n.id)),
    );
    return {
      outputs: Object.fromEntries(sinks.map((n) => [n.id, results.get(n.id)])),
      execution,
    };
  } finally {
    clearTimeout(timer);
